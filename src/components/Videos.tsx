...
export function Videos() {
  ...
  return (
    <div className="mt-6 grid gap-6 md:grid-cols-3">
      {videos.map((v, i) => (
        <Reveal key={v.id} delay={i * 110} className="flex flex-col border border-rule bg-white">
          ...
          <p className="p-6 text-[16px] leading-snug tracking-tight text-ink">{v.title}</p>
        </Reveal>
      ))}
    </div>
  );
